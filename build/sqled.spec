...
if [ $? -eq 0 ]; then
    ...
    -e "s|ExecStart=|ExecStart=/bin/sh -c 'exec $RPM_INSTALL_PREFIX\/bin\/sqled --config $RPM_INSTALL_PREFIX\/etc\/sqled.yml --pidfile=$RPM_INSTALL_PREFIX\/sqled.pid 1>$RPM_INSTALL_PREFIX\/std.log 2>$RPM_INSTALL_PREFIX\/err.log'|g" \
