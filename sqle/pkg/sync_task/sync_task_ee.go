...
func EnableSyncInstanceTask(ctx context.Context) {
	...
	for _, syncTask := range syncTasks {
		...
		syncInstance := NewSyncInstanceTask(newLog, syncTask.Source, syncTask.URL, syncTask.Version, syncTask.DbType, syncTask.RuleTemplate.Name)
...
func NewSyncInstanceTask(log *logrus.Entry, source, url, dmpVersion, dbType, ruleTemplateName string) SyncInstanceTask {
	switch source {
