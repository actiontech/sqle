...
func init() {
	sqleMiddleware.ApiInterfaceInfoList = append(sqleMiddleware.ApiInterfaceInfoList, []sqleMiddleware.ApiInterfaceInfo{
		...
		{
			RouterPath:               "/v1/projects/:project_name/audit_whitelist/rule_exceptions",
			Method:                   http.MethodPost,
			OperationType:            model.OperationRecordTypeSQLRuleException,
			OperationAction:          model.OperationRecordActionCreateSQLRuleException,
			GetProjectAndContentFunc: getProjectAndContentFromCreatingSQLRuleException,
		},
		{
			RouterPath:               "/v1/projects/:project_name/audit_whitelist/rule_exceptions/:sql_rule_exception_id",
			Method:                   http.MethodDelete,
			OperationType:            model.OperationRecordTypeSQLRuleException,
...
func getProjectAndContentFromCreatingSQLRuleException(c echo.Context) (string, string, error) {
	req := new(CreateSQLRuleExceptionReqV1)
	if err := marshalRequestBody(c, req); err != nil {
		return "", "", err
	}
	return c.Param("project_name"), fmt.Sprintf("添加单规则例外，数据源ID：%d，SQL指纹：%s，规则名：%s，规则原级别：%s，添加原因：%s", req.InstanceID, req.SQLFingerprint, req.RuleName, req.RuleLevel, req.Reason), nil
}

func getProjectAndContentFromDeletingSQLRuleException(c echo.Context) (string, string, error) {
	projectName := c.Param("project_name")
	projectUid, err := dms.GetProjectUIDByName(context.TODO(), projectName)
	if err != nil {
		return "", "", err
	}
	sqlRuleException, exist, err := model.GetStorage().GetSQLRuleExceptionByIDAndProjectID(c.Param("sql_rule_exception_id"), model.ProjectUID(projectUid))
	if err != nil {
		return "", "", err
	}
	if !exist {
		return projectName, fmt.Sprintf("取消单规则例外，例外ID：%s，取消结果：例外不存在或已取消", c.Param("sql_rule_exception_id")), nil
	}
	return projectName, fmt.Sprintf("取消单规则例外，例外ID：%d，数据源ID：%d，SQL指纹：%s，规则名：%s，规则原级别：%s，添加原因：%s，添加人：%s，添加时间：%s", sqlRuleException.ID, sqlRuleException.InstanceID, sqlRuleException.SQLFingerprint, sqlRuleException.RuleName, sqlRuleException.RuleLevel, sqlRuleException.Reason, sqlRuleException.CreatedBy, sqlRuleException.CreatedAt.Format(time.RFC3339)), nil
}
...
var typeNameDescMap = map[string]string{
	...
	model.OperationRecordTypeSQLRuleException:    "SQL审核单规则例外",
}
...
var actionNameDescMap = map[string]string{
	...
	model.OperationRecordActionCreateSQLRuleException:      "添加单规则例外",
	model.OperationRecordActionDeleteSQLRuleException:      "取消单规则例外",
}
