...
func testCodingAuditConfigV1(c echo.Context) error {
	...
	_, err = coding.NewCodingClient(codingCfg.AppKey, codingCfg.AppSecret).CreateIssue(coding.CreateIssueRequestBody{Name: issueName, Priority: CodingPriorityLow.Weight(), ProjectName: req.CodingProjectName, Type: string(CodingTypeMission), Description: description, WorkingHours: 8})
	if err != nil {
		return controller.JSONBaseErrorReq(c, err)
	}
	return c.JSON(http.StatusOK, &TestCodingConfigResV1{
