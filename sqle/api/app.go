...
func StartApi(net *gracenet.Net, exitChan chan struct{}, config *config.SqleOptions, swaggerYaml []byte) {
	...
	v1ProjectViewRouter := v1Router.Group("/projects", sqleMiddleware.ProjectMemberViewAllowed())
	{
		...
		v1ProjectViewRouter.GET("/:project_name/workflow_templates", v1.GetWorkflowTemplates)
	...
	v2ProjectViewRouter := v2Router.Group("/projects", sqleMiddleware.ProjectMemberViewAllowed())
	{
		...
		v2ProjectViewRouter.POST("/:project_name/sql_optimization_records/:optimization_record_id/feedback", v2.AddOptimizedSQLFeedback)
		v2ProjectViewRouter.PATCH("/:project_name/sql_optimization_records/:optimization_record_id/feedback/:feedback_id", v2.UpdateOptimizedSQLFeedback)
		v2ProjectViewRouter.DELETE("/:project_name/sql_optimization_records/:optimization_record_id/feedback/:feedback_id", v2.DeleteOptimizedSQLFeedback)
	}

	{
		...
		v1Router.GET("/tasks/audits/:task_id/sqls/:number/rewrite/status", v1.GetAsyncRewriteTaskStatus)
		...
		v1Router.POST("/sql_lineage_analysis", v1.SQLLineageAnalyze)
		// 企业公告
