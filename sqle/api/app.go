...
func StartApi(net *gracenet.Net, exitChan chan struct{}, config config.SqleConfig) {
	...
	v2Router.Use(sqleMiddleware.JWTTokenAdapter(), middleware.JWT([]byte(utils.JWTSecret)), sqleMiddleware.VerifyUserIsDisabled())
	...
	// v1 admin api, just admin user can access.
	{
		...
		v1Router.GET("/configurations/license/parse_result", v1.ParseLicense, AdminUserAllowed())
