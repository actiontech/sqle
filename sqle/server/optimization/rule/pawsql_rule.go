...
var ruleMapping map[string]string // pawsql的ruleCode与plugin的rule name映射关系
...
func MergeRulesAndPower(pluginRules []*driverV2.Rule) []*driverV2.Rule {
	...
	// 仅有重写能力的规则
	for _, pawsqlRule := range PawSqlRuleHandler {
		pawsqlRule.Rule.RewritePower = "true"
		pawsqlRule.Rule.AuditPower = "false"
		if _, exist := rulesMap[pawsqlRule.Rule.Name]; !exist {
			rule := pawsqlRule.Rule
