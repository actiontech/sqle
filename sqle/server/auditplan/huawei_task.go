...
func (at *huaweiRdsMySQLTask) collectorDo() {
	...
	//4. Merge SQL
	mergedSlowSqls := at.mergeSQLsFromHuaweiCloud(slowSqls)
...
func (at *huaweiRdsMySQLTask) mergeSQLsFromHuaweiCloud(sqls []SqlFromHuaweiCloud) []sqlInfo {
	sqlInfos := []sqlInfo{}
	...
	for _, sql := range sqls {
		...
		if err != nil {
			at.logger.Warnf("fingerprint err: %v sql: %s", err, sql.sql)
			continue
		}
