...
func (at *PolarDBMysqlSlowLogTask) collectorDo() {
	...
	for {
		...
		var pageNum int32 = 1
		for {
			sqls, err := at.pullSlowLogs(client, regionId, dbClusterId, startTime, now, int32(pageSize), pageNum)
			if err != nil {
...
func (at *PolarDBMysqlSlowLogTask) pullSlowLogs(client *polardb20170801.Client, regionId, dbClusterId string, startTime, endTime time.Time, pageSize, pageNum int32) (sqls []string, err error) {
...
func (at *PolarDBMysqlSlowLogTask) convertRawSQLToModelSQLs(sqls []string, now time.Time) []*model.AuditPlanSQLV2 {
	as := make([]*model.AuditPlanSQLV2, len(sqls))
