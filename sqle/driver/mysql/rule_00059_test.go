...
func NewMySQLInspectOnRuleSQLE00059(t *testing.T, schemaName, tableName string, rows string) *MysqlDriverImpl {
	e, handler, err := executor.NewMockExecutor()
	assert.NoError(t, err)

	inspect := NewMockInspect(e)

	handler.ExpectQuery(regexp.QuoteMeta(fmt.Sprintf("show table status from `%v` where name = '%v'", schemaName, tableName))).
		WillReturnRows(sqlmock.NewRows([]string{"Rows"}).AddRow(rows))
...
func TestRuleSQLE00059(t *testing.T) {
	...
	ruleParams := []interface{}{"1000000"}

	// Alter table, no modify/change column
	i := NewMySQLInspectOnRuleSQLE00059(t, "exist_db", "exist_tb_1", "2000000")
	...
	i = NewMySQLInspectOnRuleSQLE00059(t, "exist_db", "exist_tb_1", "50")
