...
var (
	...
	Rule00010Annotation   = &i18n.Message{ID: "Rule00010Annotation", Other: "在MySQL中删除已有主键代价高昂，极易引起业务阻塞、故障；开启该规则，SQLE将提醒删除主键为高危操作"}
	...
	Rule00013Desc         = &i18n.Message{ID: "Rule00013Desc", Other: "精确浮点数建议使用DECIMAL"}
	Rule00013Annotation   = &i18n.Message{ID: "Rule00013Annotation", Other: "对于浮点数运算，DECIMAL精确度较高"}
	Rule00013Message      = &i18n.Message{ID: "Rule00013Message", Other: "精确浮点数建议使用DECIMAL. 不符合规定的字段: %v"}
	...
	Rule00016_1Desc       = &i18n.Message{ID: "Rule00016_1Desc", Other: "BLOB 和 TEXT 类型的字段不建议设置为 NOT NULL"}
	Rule00016_1Annotation = &i18n.Message{ID: "Rule00016_1Annotation", Other: "BLOB 和 TEXT 类型的字段无法指定默认值，如插入数据不指定字段默认为NULL，如果添加了 NOT NULL 限制，写入数据时又未对该字段指定值会导致写入失败"}
	Rule00016_1Message    = &i18n.Message{ID: "Rule00016_1Message", Other: "BLOB 和 TEXT 类型的字段不建议设置为 NOT NULL. 不符合规定的字段: %v"}
	Rule00017Desc         = &i18n.Message{ID: "Rule00017Desc", Other: "不建议使用 BLOB 或 TEXT 类型"}
	Rule00017Annotation   = &i18n.Message{ID: "Rule00017Annotation", Other: "BLOB 或 TEXT 类型消耗大量的网络和IO带宽，同时在该表上的DML操作都会变得很慢"}
	Rule00017Message      = &i18n.Message{ID: "Rule00017Message", Other: "不建议使用 BLOB 或 TEXT 类型. 不符合规定的字段: %v"}
	...
	Rule00022Desc         = &i18n.Message{ID: "Rule00022Desc", Other: "表的列数不建议超过阈值"}
	Rule00022Annotation   = &i18n.Message{ID: "Rule00022Annotation", Other: "避免在OLTP系统上做宽表设计，后期对性能影响很大；具体规则阈值可根据业务需求调整，默认值：40"}
	Rule00022Message      = &i18n.Message{ID: "Rule00022Message", Other: "表的列数不建议超过阈值. 阈值: %v"}
	Rule00022Params1      = &i18n.Message{ID: "Rule00022Params1", Other: "最大列数"}
	Rule00023Desc         = &i18n.Message{ID: "Rule00023Desc", Other: "主键包含的列数不建议超过阈值"}
	Rule00023Annotation   = &i18n.Message{ID: "Rule00023Annotation", Other: "主建中的列过多，会导致二级索引占用更多的空间，同时增加索引维护的开销；具体规则阈值可根据业务需求调整，默认值：2"}
	...
	Rule00024Desc         = &i18n.Message{ID: "Rule00024Desc", Other: "不建议使用 SET 类型"}
	Rule00024Annotation   = &i18n.Message{ID: "Rule00024Annotation", Other: "集合的修改需要重新定义列，后期修改的代价大，建议在业务层实现"}
	Rule00024Message      = &i18n.Message{ID: "Rule00024Message", Other: "不建议使用 SET 类型. 不符合规定的字段: %v"}
	Rule00025Desc         = &i18n.Message{ID: "Rule00025Desc", Other: "TIMESTAMP 类型的列必须添加默认值"}
	Rule00025Annotation   = &i18n.Message{ID: "Rule00025Annotation", Other: "TIMESTAMP 类型的列添加默认值，可避免出现全为0的日期格式与业务预期不符"}
	Rule00025Message      = &i18n.Message{ID: "Rule00025Message", Other: "TIMESTAMP 类型的列必须添加默认值. 不符合规定的字段: %v"}
	...
	Rule00027Desc         = &i18n.Message{ID: "Rule00027Desc", Other: "列定义建议加注释"}
	Rule00027Annotation   = &i18n.Message{ID: "Rule00027Annotation", Other: "MySQL列添加注释能够使列的意义更明确，方便日后的维护。"}
	Rule00027Message      = &i18n.Message{ID: "Rule00027Message", Other: "列定义建议加注释. 不符合规定的字段: %v"}
	Rule00028Desc         = &i18n.Message{ID: "Rule00028Desc", Other: "除了自增列和blob/text类型的列, 每个列都必须添加默认值"}
	Rule00028Annotation   = &i18n.Message{ID: "Rule00028Annotation", Other: "列添加默认值，可避免列为NULL值时对查询的影响"}
	Rule00028Message      = &i18n.Message{ID: "Rule00028Message", Other: "除了自增列和blob/text类型的列, 每个列都必须添加默认值. 不符合规定的字段: %v"}
	...
	Rule00033Desc         = &i18n.Message{ID: "Rule00033Desc", Other: "建表DDL必须包含更新时间字段, 默认值为CURRENT_TIMESTAMP, ON UPDATE值为CURRENT_TIMESTAMP"}
	Rule00033Annotation   = &i18n.Message{ID: "Rule00033Annotation", Other: "使用更新时间字段，有利于问题查找跟踪和检索数据，同时避免后期对数据生命周期管理不便 ，默认值为UPDATE_TIME可保证时间的准确性"}
	Rule00033Message      = &i18n.Message{ID: "Rule00033Message", Other: "建表DDL必须包含更新时间字段, 默认值为CURRENT_TIMESTAMP, ON UPDATE值为CURRENT_TIMESTAMP. 更新时间字段名: %v"}
	...
	Rule00036Desc         = &i18n.Message{ID: "Rule00036Desc", Other: "禁止将BLOB类型的列加入索引"}
	Rule00036Annotation   = &i18n.Message{ID: "Rule00036Annotation", Other: "BLOB类型属于大字段类型，作为索引会占用很大的存储空间"}
	Rule00036Message      = &i18n.Message{ID: "Rule00036Message", Other: "BLOB类型属于大字段类型，作为索引会占用很大的存储空间"}
	...
	Rule00039Annotation   = &i18n.Message{ID: "Rule00039Annotation", Other: "为了提高查询效率，建议在执行SQL时优先使用区分度高的索引字段。区分度高的索引可以更快地定位数据，减少不必要的数据扫描，从而加速查询响应时间。规则检查将会计算候选索引字段的区分度，如果索引的区分度低于设定的阈值，则建议调整索引策略，默认值：0.7"}
	Rule00039Message      = &i18n.Message{ID: "Rule00039Message", Other: "索引列 %v 未超过区分度阈值 %v, 不建议选为索引"}
	...
	Rule00043Annotation   = &i18n.Message{ID: "Rule00043Annotation", Other: "复合索引会根据索引列数创建对应组合的索引，列数越多，创建的索引越多，每个索引都会增加磁盘空间的开销，同时增加索引维护的开销；具体规则阈值可以根据业务需求调整，默认值：2"}
	...
	Rule00059Desc         = &i18n.Message{ID: "Rule00059Desc", Other: "禁止修改大表字段类型"}
	...
	Rule00059Message      = &i18n.Message{ID: "Rule00059Message", Other: "禁止修改大表字段类型，表大小阈值: %v GB"}
	...
	Rule00064Annotation   = &i18n.Message{ID: "Rule00064Annotation", Other: "建立索引时没有限制索引的大小，索引长度会根据该字段实际存储的值来计算，VARCHAR 定义的长度越长，导致业务写入的内容越多，则建立的索引其存储大小将会越大，默认值：767"}
	...
	Rule00071Annotation   = &i18n.Message{ID: "Rule00071Annotation", Other: "业务逻辑与删除列依赖未完全消除，列被删除后可能导致程序异常（无法正常读写）的情况；开启该规则，SQLE将提醒删除列为高危操作"}
	Rule00071Message      = &i18n.Message{ID: "Rule00071Message", Other: "禁止进行删除列的操作"}
	Rule00072Desc         = &i18n.Message{ID: "Rule00072Desc", Other: "禁止进行删除外键的操作"}
	Rule00072Annotation   = &i18n.Message{ID: "Rule00072Annotation", Other: "删除已有约束会影响已有业务逻辑；开启该规则，SQLE将提醒删除外键为高危操作"}
	Rule00072Message      = &i18n.Message{ID: "Rule00072Message", Other: "禁止进行删除外键的操作"}
	Rule00073Desc         = &i18n.Message{ID: "Rule00073Desc", Other: "不建议修改表的默认字符集"}
	Rule00073Annotation   = &i18n.Message{ID: "Rule00073Annotation", Other: "修改表的默认字符集，只会影响后续新增的字段，不会修表已有字段的字符集；如需修改整张表所有字段的字符集建议开启此规则"}
	Rule00073Message      = &i18n.Message{ID: "Rule00073Message", Other: "不建议修改表的默认字符集"}
	Rule00074Desc         = &i18n.Message{ID: "Rule00074Desc", Other: "禁止使用RENAME或CHANGE对表名字段名进行修改"}
	Rule00074Annotation   = &i18n.Message{ID: "Rule00074Annotation", Other: "RENAME/CHANGE 表名/列名会对线上业务不停机发布造成影响，如需这种操作应当DBA手工干预"}
	Rule00074Message      = &i18n.Message{ID: "Rule00074Message", Other: "禁止使用RENAME或CHANGE对表名字段名进行修改"}
	...
	Rule00085Annotation   = &i18n.Message{ID: "Rule00085Annotation", Other: "MySQL需要单独维护重复的索引，冗余索引增加维护成本，影响更新性能"}
	...
	Rule00098Desc         = &i18n.Message{ID: "Rule00098Desc", Other: "避免在单个SQL语句中对同一张表进行多次连接或查询"}
	...
	Rule00098Message      = &i18n.Message{ID: "Rule00098Message", Other: "避免在单个SQL语句中对同一张表进行多次连接或查询. 违反规则的表名: %s"}
	Rule00098Params1      = &i18n.Message{ID: "Rule00098Params1", Other: "max_table_join_count"}
	Rule00099Desc         = &i18n.Message{ID: "Rule00099Desc", Other: "不建议使用SELECT FOR UPDATE"}
	...
	Rule00099Message      = &i18n.Message{ID: "Rule00099Message", Other: "不建议使用SELECT FOR UPDATE"}
	...
	Rule00101Desc         = &i18n.Message{ID: "Rule00101Desc", Other: "SELECT 语句不能有ORDER BY"}
	Rule00101Annotation   = &i18n.Message{ID: "Rule00101Annotation", Other: "ORDER BY 对查询性能影响较大，建议将排序部分放到业务处理。"}
	Rule00101Message      = &i18n.Message{ID: "Rule00101Message", Other: "SELECT 语句不能有ORDER BY"}
	...
	Rule00113Annotation   = &i18n.Message{ID: "Rule00113Annotation", Other: "SQL查询条件中存在NOT IN、NOT LIKE、NOT EXISTS、不等于等负向查询条件，将导致全表扫描，出现慢SQL"}
	...
	Rule00115Annotation   = &i18n.Message{ID: "Rule00115Annotation", Other: "当查询条件包含联合索引的最左侧字段时，查询语句才能更好的利用索引的特性：有序性、过滤性等"}
	...
	Rule00119Annotation   = &i18n.Message{ID: "Rule00119Annotation", Other: "在5.7中，MySQL默认会对’GROUP BY col1, …’按如下顺序’ORDER BY col1,…’隐式排序，导致产生无谓的排序，带来额外的开销，影响SQL执行效率；在8.0中，则不会出现这种情况。如果不需要排序建议显示添加’ORDER BY NULL’"}
	...
	Rule00123Annotation   = &i18n.Message{ID: "Rule00123Annotation", Other: "TRUNCATE是DDL，执行后数据默认隐式提交，无法回滚，在没有备份的场景下，谨慎使用TRUNCATE"}
	...
	Rule00131Annotation   = &i18n.Message{ID: "Rule00131Annotation", Other: "使用 ORDER BY RAND() 会导致 MySQL 生成临时表并进行完整的表扫描和排序，这在处理大数据量时会显著增加查询时间和服务器负载。建议采用更高效的随机数据检索方法，如利用主键或其他索引实现快速随机访问。"}
	...
	Rule00171Desc         = &i18n.Message{ID: "Rule00171Desc", Other: "建表DDL必须包含创建时间字段且默认值为CURRENT_TIMESTAMP"}
	Rule00171Annotation   = &i18n.Message{ID: "Rule00171Annotation", Other: "使用CREATE_TIME字段，有利于问题查找跟踪和检索数据，同时避免后期对数据生命周期管理不便 ，默认值为CURRENT_TIMESTAMP可保证时间的准确性"}
	Rule00171Message      = &i18n.Message{ID: "Rule00171Message", Other: "建表DDL必须包含创建时间字段且默认值为CURRENT_TIMESTAMP"}
	Rule00171Params1      = &i18n.Message{ID: "Rule00171Params1", Other: "创建时间字段名"}
	...
	Rule00178Desc         = &i18n.Message{ID: "Rule00178Desc", Other: "不建议对表进行全表排序操作"}
	Rule00178Annotation   = &i18n.Message{ID: "Rule00178Annotation", Other: "SQL语句存在全表排序操作，无过滤条件，也就是WHERE 必须显式指定过滤条件"}
	Rule00178Message      = &i18n.Message{ID: "Rule00178Message", Other: "不建议对表进行全表排序操作"}
