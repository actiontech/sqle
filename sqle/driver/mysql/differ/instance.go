...
func Schemas(ignoreSysDatabase bool, conn *executor.Executor, schemaInfos []*driverV2.DatabasSchemaInfo) ([]*Schema, error) {
	...
	for i, schemaName := range schemaInfos {
		schemaNames[i] = schemaName.ScheamName
	}
	...
	for n, rawSchema := range ret {
		...
		if len(procedureNames) > 0 {
			schemas[n].Routines, err = querySchemaRoutines(conn, rawSchema.Name, addQuotes(procedureNames), driverV2.ObjectType_PROCEDURE, flavor)
			if err != nil {
				return nil, err
			}
		}
		...
		if len(functionNames) > 0 {
			schemas[n].Routines, err = querySchemaRoutines(conn, rawSchema.Name, addQuotes(functionNames), driverV2.ObjectType_FUNCTION, flavor)
			if err != nil {
				return nil, err
			}
		}
...
func getObjetNamesBySchema(schemaName string, objectType string, schemaInfos []*driverV2.DatabasSchemaInfo) []string {
	...
	for _, schemaInfo := range schemaInfos {
		if schemaInfo.ScheamName == schemaName {
...
func addQuotes(arr []string) []string {
	quotedArr := make([]string, len(arr))
	for i, str := range arr {
		quotedArr[i] = fmt.Sprintf("'%s'", str)
	}
	return quotedArr
}
