...
func NewMySQLInspectOnRuleSQLE00096(t *testing.T, tableNames ...string) *MysqlDriverImpl {
...
func TestRuleSQLE00096(t *testing.T) {
	...
	runSingleRuleInspectCase(rule, t, "select...where use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE id = 1;
	`, newTestResult())

	//select...where not use leftmost index
	runSingleRuleInspectCase(rule, t, "select...where not use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE v2 = 1;
	`, newTestResult().addResult(ruleName, "v2"))

	//select...where 1=1
	runSingleRuleInspectCase(rule, t, "select...where 1=1", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE 1=1;
	`, newTestResult())

	//select...where True
	runSingleRuleInspectCase(rule, t, "select...where True", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE True;
	`, newTestResult())

	//select...where use leftmost index, with group by
	runSingleRuleInspectCase(rule, t, "select...where use leftmost index, with group by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE id = 1 GROUP BY id;
	`, newTestResult())

	//select...where not use leftmost index, with group by
	runSingleRuleInspectCase(rule, t, "select...where not use leftmost index, with group by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE v2 = 1 GROUP BY id;
	`, newTestResult().addResult(ruleName, "v2"))

	//select...where use leftmost index, with order by
	runSingleRuleInspectCase(rule, t, "select...where use leftmost index, with order by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE id = 1 ORDER BY id;
	`, newTestResult())

	//select...where not use leftmost index, with order by
	runSingleRuleInspectCase(rule, t, "select...where not use leftmost index, with order by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE v2 = 1 ORDER BY id;
	`, newTestResult().addResult(ruleName, "v2"))

	//select...where 1=1, with group by
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with group by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE 1=1 GROUP BY id;
	`, newTestResult())

	//select...where 1=1, with group by, not use leftmost index
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with group by, not use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE 1=1 GROUP BY v2;
	`, newTestResult().addResult(ruleName, "v2"))

	//select...where 1=1, with order by
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with order by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	SELECT * FROM exist_db.exist_tb_1 WHERE 1=1 ORDER BY id;
	`, newTestResult())

	//select...where 1=1, with order by, not use leftmost index
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with order by, not use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1"), `
	...
	runSingleRuleInspectCase(rule, t, "select...where normal, with join, on condition not use leftmost index, with group by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	...
	runSingleRuleInspectCase(rule, t, "select...where normal, with join, on condition not use leftmost index, with order by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	...
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with join, on condition not use leftmost index, with group by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	...
	runSingleRuleInspectCase(rule, t, "select...where 1=1, with join, on condition not use leftmost index, with order by", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	...
	runSingleRuleInspectCase(rule, t, "insert...select...where use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3"), `
	INSERT INTO exist_db.exist_tb_1 SELECT * FROM exist_db.exist_tb_3 WHERE id = 1;
	`, newTestResult())

	//insert...select...where not use leftmost index
	runSingleRuleInspectCase(rule, t, "insert...select...where not use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3"), `
	INSERT INTO exist_db.exist_tb_1 SELECT * FROM exist_db.exist_tb_3 WHERE v2 = 1;
	`, newTestResult().addResult(ruleName, "v2"))

	//insert...select...where 1=1
	runSingleRuleInspectCase(rule, t, "insert...select...where 1=1", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3"), `
	...
	runSingleRuleInspectCase(rule, t, "insert...select...where use leftmost index, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3", "exist_tb_4"), `
	...
	runSingleRuleInspectCase(rule, t, "insert...select...where not use leftmost index, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3", "exist_tb_4"), `
	...
	runSingleRuleInspectCase(rule, t, "insert...select...where 1=1, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_3", "exist_tb_4"), `
	...
	runSingleRuleInspectCase(rule, t, "union...where use leftmost index", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	SELECT * FROM (SELECT id FROM exist_db.exist_tb_1 WHERE id = 1 UNION ALL SELECT id FROM exist_db.exist_tb_2 WHERE id = 1) a;
	`, newTestResult())

	//union...where 1=1
	runSingleRuleInspectCase(rule, t, "union...where 1=1", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2"), `
	...
	runSingleRuleInspectCase(rule, t, "union...where use leftmost index, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2", "exist_tb_3", "exist_tb_4"), `
	...
	runSingleRuleInspectCase(rule, t, "union...where not use leftmost index, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2", "exist_tb_3", "exist_tb_4"), `
	...
	runSingleRuleInspectCase(rule, t, "union...where 1=1, with join", NewMySQLInspectOnRuleSQLE00096(t, "exist_tb_1", "exist_tb_2", "exist_tb_3", "exist_tb_4"), `
