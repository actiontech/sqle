...
func TestRuleSQLE00022(t *testing.T) {
	...
	runAIRuleCase(rule, t, "case 1: CREATE INDEX on table_users.username with skewness below threshold",
	...
	runAIRuleCase(rule, t, "case 2: CREATE INDEX on table_orders.order_id with skewness above threshold",
		...
		[]*AIMockSQLExpectation{
			...
			},
			{
				...
				Rows:  sqlmock.NewRows([]string{"record_count"}).AddRow(40000),
			},
		}, newTestResult().addResult(ruleName, "order_id", "0.7"))

	runAIRuleCase(rule, t, "case 3: ALTER TABLE table_products ADD INDEX idx_product_code(product_code) with skewness below threshold",
	...
	runAIRuleCase(rule, t, "case 4: ALTER TABLE table_customers ADD INDEX idx_email(email) with skewness above threshold",
	...
	runAIRuleCase(rule, t, "case 5: SELECT query on table_employees with indexed field salary having skewness below threshold",
	...
	runAIRuleCase(rule, t, "case 6: SELECT query on table_inventory with indexed field sku having skewness above threshold",
	...
	runAIRuleCase(rule, t, "case 7: UPDATE statement on table_accounts with indexed field account_id having skewness below threshold",
	...
	runAIRuleCase(rule, t, "case 9: ALTER TABLE customers ADD INDEX idx_age_customers(age) with skewness above threshold (从xml中补充)",
		...
		[]*AIMockSQLExpectation{
			...
			},
			{
				...
				Rows:  sqlmock.NewRows([]string{"record_count"}).AddRow(45000),
			},
		}, newTestResult().addResult(ruleName))

	runAIRuleCase(rule, t, "case 10: CREATE INDEX on customers.name with skewness below threshold (从xml中补充)",
	...
	runAIRuleCase(rule, t, "case 11: SELECT query on customers with indexed field name having skewness below threshold (从xml中补充)",
	...
	runAIRuleCase(rule, t, "case 12: SELECT query on customers with indexed field age having skewness above threshold (从xml中补充)",
