...
func mockPrefixIndexOptimizeResult(caseName string, c optimizerTestContent, t *testing.T) []*OptimizeResult {
	return mockOptimizeResultWithAdvisor(c.sql, c.maxColumn, c.queryResults, caseName, t,
		func(optimizer *indexOptimizer) selectAdvisorVisitor {
			return selectAdvisorVisitor{
				...
				whereAdvisorVisitor: whereAdvisorVisitor{
					...
					prefixIndexAdvisor: newPrefixIndexAdvisor(optimizer.drivingTableSource),
				},
				advices: make([]*OptimizeResult, 0),
			}
		},
		...
		func(optimizer *indexOptimizer) selectAdvisorVisitor {
			return selectAdvisorVisitor{
				...
				threeStarAdvisor: newThreeStarAdvisor(
					optimizer.sqlContext, optimizer.log,
					optimizer.drivingTableSource, optimizer.drivingTableCreateStmt, optimizer.originNode,
					optimizer.compositeIndexMaxColumn,
				),
				advices: make([]*OptimizeResult, 0),
			}
		},
	...
	return mockOptimizeResultWithAdvisor(c.sql, c.maxColumn, c.queryResults, caseName, t,
		func(optimizer *indexOptimizer) selectAdvisorVisitor {
			return selectAdvisorVisitor{
				...
				whereAdvisorVisitor: whereAdvisorVisitor{
					advices:              make([]*OptimizeResult, 0),
					functionIndexAdvisor: newFunctionIndexAdvisor(optimizer.sqlContext, optimizer.log, optimizer.drivingTableSource),
				},
				advices: make([]*OptimizeResult, 0),
			}
		},
...
func mockOptimizeResultWithAdvisor(sql string, maxColumn int, queryResults []*queryResult, caseName string, t *testing.T, f func(*indexOptimizer) selectAdvisorVisitor) []*OptimizeResult {
	...
	optimizer := NewIndexOptimizer(log.NewEntry(), impl.Ctx, node, WithMaxColumn(maxColumn))
	if !assert.Truef(t, optimizer.canOptimize(), caseName) {
		assert.FailNow(t, "optimizer can not optimize")
	}
	optimizer.loadTablesShouldBeOptimize()
	if !assert.Falsef(t, optimizer.hasNoTablesToOptimize(), caseName) {
		assert.FailNow(t, "optimizer has no table to optimize")
	}

	advisor := f(optimizer)

	optimizer.originNode.Accept(&advisor)
	result := advisor.advices
	return result
}
...
func (testCases optimizerTestCaseMap) testOne(caseName string, testFunc func(caseName string, c optimizerTestContent, t *testing.T) []*OptimizeResult, t *testing.T) {
	...
	assert.Equalf(t, result, c.expectResults, caseName)
}
